...
export const protobufPackage = "mmuljiic.blog.blog";
...
function createBaseQueryPostsResponse(): QueryPostsResponse {
  return { title: "", body: "", Post: [], pagination: undefined };
}
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("mmuljiic.blog.blog.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Posts(request: QueryPostsRequest): Promise<QueryPostsResponse> {
    const data = QueryPostsRequest.encode(request).finish();
    const promise = this.rpc.request("mmuljiic.blog.blog.Query", "Posts", data);
  ...
  Comment(request: QueryGetCommentRequest): Promise<QueryGetCommentResponse> {
    ...
    const promise = this.rpc.request("mmuljiic.blog.blog.Query", "Comment", data);
  ...
  CommentAll(request: QueryAllCommentRequest): Promise<QueryAllCommentResponse> {
    ...
    const promise = this.rpc.request("mmuljiic.blog.blog.Query", "CommentAll", data);
